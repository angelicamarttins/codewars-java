...
public class PeteBaker {
  ...
  */
  ...
  public static int cakes(Map<String, Integer> recipe, Map<String, Integer> available) {
    List<Integer> numberOfCakes = new ArrayList<>();

    recipe.forEach((key, value) -> {
      Optional<Integer> availableIngredient = Optional.ofNullable(available.get(key));

      if (availableIngredient.isEmpty()) {
        availableIngredient = Optional.of(0);
      }

      numberOfCakes.add(availableIngredient.get() / value);
    });

    return Collections.min(numberOfCakes);
  }
